...
describe("WaveFSM", () => {
  ...
  it("transitions to WAVE_ACTIVE after preparation", async () => {
  ...
  it("calls spawnWaveEnemies on entering WAVE_ACTIVE", async () => {
    ...
    const fsm = createWaveFSM(ctx);
    await fsm.start();
    await fsm.update(0.2); // prep -> WAVE_ACTIVE
  ...
  it("transitions to WAVE_COMPLETE when all enemies dead", async () => {
    const ctx = createWaveCtx({ prepDuration: 0.1 });
    const fsm = createWaveFSM(ctx);
    await fsm.start();
    await fsm.update(0.2); // -> WAVE_ACTIVE
  ...
  it("loops PREPARING -> WAVE_ACTIVE -> WAVE_COMPLETE -> PREPARING", async () => {
    const ctx = createWaveCtx({ prepDuration: 0.1 });
    const fsm = createWaveFSM(ctx);
    await fsm.start();
    ...
    await fsm.update(0.2); // -> WAVE_ACTIVE
    ctx.enemiesRemaining = 0;
    await fsm.update(0.1); // -> WAVE_COMPLETE
    await fsm.update(0.1); // -> PREPARING

    expect(fsm.currentStateId).toBe("PREPARING");
    expect(ctx.score).toBe(50);
  });

  it("goes to GAME_OVER after all waves complete", async () => {
    ...
    ctx.enemiesRemaining = 0;
    await fsm.update(0.1); // -> WAVE_COMPLETE
    await fsm.update(0.1); // -> GAME_OVER

    expect(fsm.currentStateId).toBe("GAME_OVER");
  });

  it("goes to GAME_OVER if base destroyed during WAVE_ACTIVE", async () => {
    const ctx = createWaveCtx({ prepDuration: 0.1 });
    const fsm = createWaveFSM(ctx);
    await fsm.start();
    await fsm.update(0.2); // -> WAVE_ACTIVE
  ...
  it("goes to GAME_OVER if base destroyed during PREPARING", async () => {
  ...
  it("increases enemy count each wave", async () => {
    const spawns: { wave: number; count: number }[] = [];
    const ctx = createWaveCtx({
      prepDuration: 0.1,
      spawnWaveEnemies: (w, c) => spawns.push({ wave: w, count: c }),
