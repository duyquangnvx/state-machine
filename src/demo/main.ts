...
import type { TowerContext, EnemyRef } from "./tower/TowerContext.js";
...
function render(tick: number): void {
  ...
  console.log(
    `Wave: ${waveState} (${waveCtx.enemiesRemaining} enemies remaining)`,
  );

  for (const t of towers) {
    const state = t.fsm.isStarted ? t.fsm.currentStateId : "STOPPED";
    const target =
      t.ctx.targetEnemyId !== null ? ` -> enemy_${t.ctx.targetEnemyId}` : "";
    console.log(
  ...
  const alive = enemies.filter(
    (e) => e.fsm.isStarted && e.fsm.currentStateId !== "DEAD",
  );
